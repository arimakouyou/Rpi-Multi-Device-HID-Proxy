...
def load_config(config_path=None):
    """設定ファイルを読み込む"""
    # config_pathが指定されていない場合、複数の場所で設定ファイルを検索
    if config_path is None:
        ...
        if config_path is None:
            # どの場所にも設定ファイルが見つからない場合
            logging.warning("設定ファイルが見つかりません。以下の場所を検索しました:")
            for path in search_paths:
                logging.warning(f"  - {path}")
            config_path = search_paths[0]  # デフォルトパスを設定
    ...
    try:
        if os.path.exists(config_path):
            ...
            logging.info(f"設定ファイルの検索パス: {os.path.abspath(config_path)}")
            logging.info(f"現在の作業ディレクトリ: {os.getcwd()}")
            logging.info(f"スクリプトのディレクトリ: {os.path.dirname(os.path.abspath(__file__))}")
