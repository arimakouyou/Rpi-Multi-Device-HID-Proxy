...
| `keyboard-proxy.service` | Python + asyncio (`src/keyboard_proxy.py`) | 1 (常駐) | 全キーボードの evdev → HID 変換、GPIO ボタン処理、NeoPixel LED 制御 |
...
- キーボードは **リマップロジックが複雑** (US→JIS 変換、Shift 修飾、リマップ on/off 切替) で、Python の方が見通しよく拡張できる。GPIO/LED も Python ライブラリ (`gpiozero`, `rpi_ws281x`) との親和性が高い。
...
| LED は GPIO18 (PWM0) 占有 | `rpi_ws281x` 使用、root 必須、オーディオ出力と競合 | `src/keyboard_proxy.py:467-485` |
| GPIO ピンがハードコード | Btn1=GPIO6、Btn2=GPIO22、Btn3=GPIO17 を変更するにはソース改変が必要 | `src/keyboard_proxy.py:446-458` |
